...
NGINX_SUBPROCESS_TIMEOUT_S = 30

GATE_TEMPLATE = """# Generated by wb-homeui-backend from {source}. DO NOT EDIT.
server {{
    listen {external_port}{ssl_suffix};
    server_name _;
    # Zone in conf.d/wb-homeui-gates.conf; burst absorbs SPA storms, floods get 429.
...
class ApplyResult:
    ...
    skipped: list[str] = field(default_factory=list)  # "<file or gate name>: <reason>"
...
def render_gate(gate: Gate, https_enabled: bool) -> str:
    ...
    # (custom locations, timeouts). Included from its copy in the rendered dir, so a
    # source .inc deleted after apply cannot fail a later nginx restart; a broken one
    # is caught by nginx -t + rollback.
    extra_exists = os.path.exists(os.path.join(GATES_CONF_DIR, gate.name + ".nginx.inc"))
    rendered_extra_path = os.path.join(RENDERED_GATES_DIR, gate.name + ".nginx.inc")
    return GATE_TEMPLATE.format(
        ...
        extra_include=f"    include {rendered_extra_path};\n" if extra_exists else "",
    )
...
def _render_all_files(gates: list[Gate], https_enabled: bool) -> dict[str, str]:
    files = {}
    for gate in gates:
        files[gate.name + ".conf"] = render_gate(gate, https_enabled)
        extra_path = os.path.join(GATES_CONF_DIR, gate.name + ".nginx.inc")
        if os.path.exists(extra_path):
            with open(extra_path, "r", encoding="utf-8") as f:
                files[gate.name + ".nginx.inc"] = f.read()
    return files


def _render_bounces(gates: list[Gate], https_enabled: bool) -> str:
...
def _rendered_state_is_current(rendered: dict[str, str], bounces: str) -> bool:
    """True when the on-disk render already matches: nginx runs it, no reload needed."""
    try:
        if sorted(os.listdir(RENDERED_GATES_DIR)) != sorted(rendered):
            return False
        for file_name, content in rendered.items():
            with open(os.path.join(RENDERED_GATES_DIR, file_name), "r", encoding="utf-8") as f:
                if f.read() != content:
                    return False
        with open(BOUNCES_CONF_PATH, "r", encoding="utf-8") as f:
            return f.read() == bounces
    except OSError:
        return False


def _write_rendered_files(rendered: dict[str, str], bounces: str) -> None:
    ...
    for file_name, content in rendered.items():
        with open(os.path.join(RENDERED_GATES_DIR, file_name), "w", encoding="utf-8") as f:
            f.write(content)
    with open(BOUNCES_CONF_PATH, "w", encoding="utf-8") as f:
        f.write(bounces)


def _write_menu_items_best_effort(gates: list[Gate]) -> None:
    # Called only when the gates are live: a menu hiccup must not fail the apply.
    try:
    ...
    for attempt in range(NGINX_TEST_ATTEMPTS):
        try:
            result = subprocess.run(  # nosec B603 - fixed constant argv
                ...
                text=True,
                check=False,
                timeout=NGINX_SUBPROCESS_TIMEOUT_S,
            )
        except subprocess.TimeoutExpired:
            error = "nginx -t timed out"
        else:
...
def _reload_nginx() -> None:
    # An inactive nginx reads the rendered files when it starts; reloading it would
    # fail and needlessly roll back a good render (boot-order race with nginx.service).
    is_active = subprocess.run(  # nosec B603 - fixed constant argv
        ["/usr/bin/systemctl", "is-active", "--quiet", "nginx"],
        check=False,
        timeout=NGINX_SUBPROCESS_TIMEOUT_S,
    )
    if is_active.returncode != 0:
        logging.info("nginx is not active, skipping reload")
        return
    subprocess.run(  # nosec B603 - fixed constant argv
        ["/usr/bin/systemctl", "reload", "nginx"], check=True, timeout=NGINX_SUBPROCESS_TIMEOUT_S
    )
...
def _apply_gates_locked(https_enabled: bool) -> ApplyResult:
    ...
    rendered = _render_all_files(gates, https_enabled)
    bounces = _render_bounces(gates, https_enabled)
    if _rendered_state_is_current(rendered, bounces):
        # Typical on the startup reconcile: nginx already runs exactly this render,
        # so skip the shared nginx -t + reload instead of bouncing it every start.
        _write_menu_items_best_effort(gates)
        return ApplyResult(ok=True, gates=gates, skipped=skipped)
    with tempfile.TemporaryDirectory() as snapshot_dir:
        _snapshot_rendered_state(snapshot_dir)
        try:
            _write_rendered_files(rendered, bounces)
            ...
            # A mid-write failure (e.g. ENOSPC) must not leave a partial render behind.
            _restore_rendered_state(snapshot_dir)
            raise
    # Menu is written only after a successful reload (no items for dead gates).
    _write_menu_items_best_effort(gates)
