...
# Cloud tunnels arrive from loopback over plain HTTP; in http mode the main listener covers them.
LOOPBACK_LISTEN_TEMPLATE = "    listen 127.0.0.1:{external_port};\n"

AUTH_LOCATION_TEMPLATE = """        # Loopback is the cloud tunnel: the cloud has authorized the user already.
        satisfy any;
        allow 127.0.0.1;
        deny all;
        set $wb_role   {role};
...
def render_gate(gate: Gate, https_enabled: bool) -> str:
    ...
    return GATE_TEMPLATE.format(
        ...
        loopback_listen=(
            LOOPBACK_LISTEN_TEMPLATE.format(external_port=gate.external_port) if https_enabled else ""
        ),
        tls_include=(
