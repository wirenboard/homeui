...
import { type ArrayStore } from './array-store';
...
export const FormStringEdit = observer(({ store }: { store: StringStore }) => {
...
export const FormCheckbox = observer(({ store }: { store: BooleanStore }) => {
  const showCaption = useContext(ShowParamCaptionContext);
  return (
    <BooleanField
      title={showCaption && store.name}
...
export const FormSelect = observer(({ store, isClearable }: { store: OptionsStore; isClearable?: boolean }) => {
  const showCaption = useContext(ShowParamCaptionContext);
...
export const FormOneOf = observer(({ store }: { store: OneOfStore }) => (
...
export const FormCollapsibleTable = observer(({ store }: { store: ArrayStore }) => {
...
export const Form = observer(({ store, children }: { store: FormStore; children?: ReactNode }) => {
  const { t } = useTranslation();
  const showCaption = useContext(ShowParamCaptionContext);
  if (!store) {
    return null;
