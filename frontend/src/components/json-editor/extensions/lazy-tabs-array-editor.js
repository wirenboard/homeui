...
export function makeLazyTabsArrayEditor() {
  return class extends JSONEditor.defaults.editors['array'] {
    ...
    renderRow(i, value, initial) {
      let tabText = this.rows[i].tab_text;
      let tab = this.rows[i].tab;
      this.rows[i] = this.getElementEditor(i);
      this.row_cache[i] = this.rows[i];
      this.rows[i].tab = tab;
      this.rows[i].tab_text = tabText;
    ...
    updateTabTextContent(i, value) {
      ...
      if (schema.headerTemplate) {
        let headerTemplate = this.jsoneditor.compileTemplate(
          this.translateProperty(schema.headerTemplate),
          this.template_engine,
        );
        this.rows[i].tab_text.textContent = headerTemplate({ self: value, i0: i, i1: i + 1 });
      } else {
