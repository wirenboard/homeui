...
class Editor {
  ...
  updateState(paramValues) {
    let enable = this.shouldEnable(paramValues);
  ...
  shouldEnable(paramValues) {
    if (this.isConditionalOneOfEditor()) {
      let index = this.schema.oneOf.findIndex((schema) => {
...
class Group {
  ...
  updateState() {
    let oldState = this.isEnabled;
    this.isEnabled =
      this.params.enabledCount !== 0 ||
...
export function makeGroupsEditor() {
  ...
  return class extends JSONEditor.AbstractEditor {
    ...
    register() {
      super.register();
      if (this.editors) {
        Object.entries(this.editors).forEach(([_key, ed]) => {
          if (ed.editor) {
            ed.editor.register();
          }
        });
      ...
      if (this.schema.options && this.schema.options.wb && this.schema.options.wb.groups) {
        this.schema.options.wb.groups.forEach((groupSchema) => {
          if (
            ...
            !Array.isArray(groupSchema) &&
            groupSchema !== null
          ) {
            let group = this.groups.get(groupSchema.id);
            if (group) {
            ...
            let parentGroup = this.groups.get(groupSchema.group);
            if (!parentGroup) {
    ...
    preBuildEditors() {
      ...
      Object.entries(this.schema.properties).forEach(([key, schema]) => {
        let group = this.groups.get(schema.group);
    ...
    checkRequired(key, editor) {
      return (
        (this.schema.required && this.schema.required.includes(key)) || editor.schema.requiredProp
      );
    }
    ...
    refreshAddProperties() {
      ...
      Object.entries(this.getRootGroup().params.editors).forEach(([key, ed]) => {
        if (
          ...
          (!ed.schema.options || !ed.schema.options.show_opt_in)
        ) {
    ...
    setValue(value, initial) {
      ...
      this.disableValueUpdate = true;
      let hasChanges = true;
      for (let cycles = 0; hasChanges && cycles < 10; ++cycles) {
        Object.entries(value).forEach(([key, val]) => {
          if (this.editors.hasOwnProperty(key)) {
            let ed = this.editors[key];
            if (ed.editor) {
        ...
        Object.entries(this.getRootGroup().params.editors).forEach(([key, ed]) => {
          if (
            ...
            !this.checkRequired(key, ed.editor) &&
            ed.editor.options.show_opt_in
          ) {
    ...
    addTab(group) {
      ...
      let tab = new Tab(
        this.theme.getTab(tabText, this.getValidId(this.path + '.' + group.schema.id)),
    ...
    storeEditor(key, editor, group, schema) {
      let ed = new Editor(editor, group, schema, this.conditions);
      this.editors[key] = ed;
      return ed;
    }

    createChannels(container, group) {
      let schemas = Object.entries(group.channels.editors).sort(
        (a, b) => a[1].schema.default.channelIndex - b[1].schema.default.channelIndex,
      );
      if (schemas.length) {
        let table = this.theme.getTable();
        container.appendChild(table);
        let thead = this.theme.getTableHead();
        table.appendChild(thead);
        let header_row = this.theme.getTableRow();
        thead.appendChild(header_row);
        let row_holder = this.theme.getTableBody();
        ...
        schemas.forEach(([key, ed]) => {
          ...
          let row = row_holder.appendChild(this.theme.getTableRow());
          let ret = this.jsoneditor.createEditor(editorClass, {
          ...
          ed.editor = ret;
          ret.preBuild();
          let cell = this.theme.getTableCell();
    ...
    createTopLevelEditors(container) {
      Object.entries(this.getRootGroup().params.editors)
        ...
        .forEach(([key, ed]) => {
          let editorHolder = document.createElement('div');
    ...
    createGroups(container, group) {
      ...
      if (group.schema.description) {
        container.appendChild(
          this.theme.getFormInputDescription(this.translateProperty(group.schema.description)),
        );
      }
    ...
    onChildEditorChange(editor) {
      ...
      let ed = this.editors[editor.key];
      if (ed) {
    ...
    showEnabledTab() {
      ...
      let tab = this.tabs.find((tab) => tab.tab === this.activeTab);
    ...
    showValidationErrors(errors) {
      ...
      Object.entries(this.editors).forEach(([key, ed]) =>
        ed.setErrors(otherErrors, `${this.path}.${key}`),
      );
