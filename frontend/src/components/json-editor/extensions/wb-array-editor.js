  ...
  return class extends JSONEditor.defaults.editors['array'] {
    ...
    getItemsOptions() {
      ...
      this.schema.items.oneOf.forEach((el, i) => {
        let schema = this.jsoneditor.expandRefs(el);
        let title = 'Item';
        if (schema.title) {
    ...
    buildAddRowDialog() {
      ...
      let row = this.theme.getGridRow();
      let column = this.theme.getGridColumn();
      this.theme.setGridColumnSize(column, 12);
      let addrow_label_text = this.translate('select_type');
      ...
      this.theme.setSelectOptions(
        this.addrow_select,
        this.addrow_options.map((el) => el.value),
        this.addrow_options.map((el) => el.title),
      );
      ...
      this.addrow_save.addEventListener('click', (e) => {
        ...
        e.stopPropagation();
        this._addRowByType(
          this.addrow_options[this.addrow_select.selectedOptions[0].index].default,
        );
    ...
    _showAddRowDialog() {
      ...
      this.addrow_holder.style.top = `${
        this.add_row_button.offsetTop + this.add_row_button.offsetHeight
      }px`;
    ...
    _addRowByType(value) {
      ...
      if (this._hasWbOptions(this.schema) && this.schema.options.wb.sort_by_property) {
        ...
        const prop = this.schema.options.wb.sort_by_property;
        let index = v.findIndex((item) => this._compareBySortProp(item, valueToReplace, prop) === 1);
        if (index !== -1) {
    ...
    addRow(value, initial) {
      const i = this.rows.length;
      let row = super.addRow(value, initial);
      if (this.tabs_holder && this.theme.setTabIcon && this.schema.items.oneOf) {
        ...
        if (
          this._hasWbOptions(item_schema) &&
          item_schema.options.wb.color &&
