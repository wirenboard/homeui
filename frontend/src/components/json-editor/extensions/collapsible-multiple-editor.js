...
export function makeCollapsibleMultipleEditor() {
  return class extends JSONEditor.AbstractEditor {
    ...
    build() {
      ...
      let opt_group = undefined;
      let opt_group_id = undefined;
      ...
      this.types.forEach((type, i) => {
        ...
        option.setAttribute('value', type.display_text);
        if (
          this._hasWbOptions(type) &&
          type.options.wb.group &&
          this._hasGroups(this.schema) &&
          this.schema.options.wb.groups.indexOf(type.options.wb.group) != -1
        ) {
      ...
      if (!this.collapsed && this._isHidden(type)) {
        if (this.deprecationNotice) {
          ...
          if (this._hasWbOptions(this.schema) && this.schema.options.wb.hidden_notice) {
            ...
            this.deprecationNotice.innerHTML = this.translateProperty(
              this.schema.options.wb.hidden_notice,
            );
        ...
        if (this.type === type) {
          if (this.keep_values) {
            ...
            if (this._hasWbOptions(this.schema) && this.schema.options.wb.keep_properties) {
              let valueToKeep = editor.getDefault();
    ...
    setValue(val, initial) {
      ...
      this.types.forEach((type, i) => {
        if (this._hasOneValueRequiredStringProperty(type)) {
          if (
            this.value.hasOwnProperty(type.required[0]) &&
    ...
    setDisplayText(types) {
      ...
      types.forEach((el) => {
        ...
        if (el.title) {
          ...
          if (
            this._hasWbOptions(this.schema) &&
            this.schema.options.wb.hidden_msg &&
            this._isHidden(el)
          ) {
    ...
    showValidationErrors(errors) {
      ...
      this.editors.forEach((editor, i) => {
        if (!editor) return;
        let originalIndex = this.types[i].originalIndex;
    ...
    _isUnknownDeviceType(i) {
      ...
      return (
        props &&
        props.device_type &&
        props.device_type.enum &&
        props.device_type.enum[0] === 'unknown'
      );
    }
    ...
    _sortTypesByGroups() {
      this.types.sort((t1, t2) => {
        let gr1 = this._hasWbOptions(t1) && t1.options.wb.group;
        let gr2 = this._hasWbOptions(t2) && t2.options.wb.group;
        if (gr1 != gr2) {
          if (gr1 == undefined) {
            return 1;
          }
          ...
          let gr1i = this.schema.options.wb.groups.indexOf(t1.options.wb.group);
          let gr2i = this.schema.options.wb.groups.indexOf(t2.options.wb.group);
          if (gr1i != gr2i) {
            if (gr1i == -1) {
              return 1;
            }
