  ...
  return function (vars) {
    ...
    return (
      src
        ...
        .replace(
          /\{\{\s*if\s+([\w.\[\]]+?|".*?")\s*(==|in|intersect)\s*([\w.\[\]]+?|".*?"|\[.*?\])\s*\}\}(.*?)(?:\{\{else\}\}(.*?))?\{\{\s*endif\s*\}\}/g,
        ...
        .replace(/\{\{\s*translate\s+([\w.\[\]]+?)\s*\}\}/g, function (m, expr) {
          let v = stringify(expandVar(expr));
          return translateFn ? translateFn(v) : v;
        })
        // {{PREFIX|VARIABLE|POSTFIX}}
        .replace(
          /\{\{(?:([^{]*?)\|)?\s*([\w.\[\]]+?)\s*(?:\|([^}]*?))?\}\}/g,
          function (m, prefix, expr, suffix) {
            let v = stringify(expandVar(expr));
