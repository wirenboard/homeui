...
export default function userAgentFactory() {
  ...
  userAgentF.isMobile = {
    ...
    any: function () {
      return (
        userAgentF.isMobile.Android() ||
        userAgentF.isMobile.BlackBerry() ||
        userAgentF.isMobile.iOS() ||
        userAgentF.isMobile.Opera() ||
        userAgentF.isMobile.Windows()
      );
    },
  };
  ...
  userAgentF.vk = function () {
    ...
    var browser = {
      ...
      msie:
        (/msie/i.test(_ua) && !/opera/i.test(_ua)) || /trident\//i.test(_ua) || /edge/i.test(_ua),
      ...
      mac: /mac/i.test(_ua),
      search_bot:
        /(yandex|google|stackrambler|aport|slurp|msnbot|bingbot|twitterbot|ia_archiver|facebookexternalhit)/i.test(
          _ua
        ),
    };
  };
