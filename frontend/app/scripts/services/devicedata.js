...
function deviceDataService(mqttClient, $window) {
  ...
  function ensureDevice(id) {
    if (!devices.hasOwnProperty(id)) devices[id] = new Device(id);
    devices[id].isSystemDevice = id.startsWith('system__');
    return devices[id];
  }

  function maybeRemoveDevice(id) {
    if (!devices.hasOwnProperty(id)) return;
    if (!devices[id].explicit && !devices[id].cellIds.length) delete devices[id];
  }
  ...
  function internCell(id) {
    if (cells.hasOwnProperty(id)) return cells[id];
  ...
  function compareCellIds(idA, idB) {
    if (!cells.hasOwnProperty(idA) || !cells.hasOwnProperty(idB)) return 0;

    var cellA = cells[idA],
      cellB = cells[idB],
      r;
    if (cellA.order !== null) return cellB.order === null ? -1 : cellA.order - cellB.order;
  ...
  class Cell {
    constructor(id) {
      ...
      if (parts.length != 2) throw new Error('invalid cell id: ' + this.id);
    ...
    _removeFromDevice() {
      if (!devices.hasOwnProperty(this.deviceId)) return;
    ...
    receiveValue(newValue) {
      if (!newValue)
        ...
        // if value is removed, non-pushbutton/text cell becomes incomplete
        // so replace null by '-'
    ...
    sendValue(newValue) {
      if (!this.isComplete() || this.readOnly) return;
      if (newValue === '' && !this._isString()) newValue = this._value;
      this._setCellValue(newValue);
      mqttClient.send(this._sendValueTopic(), this._isButton() ? '1' : this.stringValue(), false);
    }
    ...
    _updateCompleteness() {
      ...
      if (this.type == 'incomplete' && this._value === null) delete cells[this.id];
    }
    ...
    setType(type) {
      ...
      if (this._value !== null) this._setCellValue(this._value);
      else if (this._isString()) this._setCellValue('');
      this._updateCompleteness();
    }
    ...
    get readOnly() {
      if (this._explicitReadOnly === null) return this._typeEntry().readOnly;
      else return this._explicitReadOnly;
    }

    setExplicitReadOnly(readOnly) {
      if (readOnly === null || readOnly === undefined) this._explicitReadOnly = null;
      else this._explicitReadOnly = !!readOnly;
    }
  ...
  mqttClient.addStickySubscription('/devices/#', msg => {
    ...
    const subscriptionHandlers = [
      ...
      },
      {
        ...
        handler(payload) {
          cellFromTopic(topic).receiveValue(payload);
        },
      },
      {
        handledTopic: cellTopicBase + '/meta',
        handler(payload) {
          cellFromTopic(topic).setMeta(payload);
        },
      },
      {
        handledTopic: cellTopicBase + '/meta/type',
        handler(payload) {
          cellFromTopic(topic).setType(payload);
        },
      },
      {
        handledTopic: cellTopicBase + '/meta/name',
        handler(payload) {
          cellFromTopic(topic).setName(payload);
        },
      },
      {
        handledTopic: cellTopicBase + '/meta/units',
        handler(payload) {
          cellFromTopic(topic).setUnits(payload);
        },
      },
      {
      ...
      },
      {
        ...
        handler(payload) {
          cellFromTopic(topic).setError(payload);
        },
      },
      {
        handledTopic: cellTopicBase + '/meta/min',
        handler(payload) {
          cellFromTopic(topic).setMin(payload);
        },
      },
      {
        handledTopic: cellTopicBase + '/meta/max',
        handler(payload) {
          cellFromTopic(topic).setMax(payload);
        },
      },
      {
        handledTopic: cellTopicBase + '/meta/precision',
        handler(payload) {
          cellFromTopic(topic).setStep(payload);
        },
      },
      {
        handledTopic: cellTopicBase + '/meta/order',
        handler(payload) {
          cellFromTopic(topic).setOrder(payload);
        },
      },
    ];
  ...
  function filterCellIds(func) {
    ...
    Object.keys(devices)
      ...
      .forEach(devId => {
        ...
        if (func) devCellIds = devCellIds.filter(id => func(cells[id]));
  ...
  class CellProxy {
    ...
    sendValue(newValue) {
      if (this.gotCell()) this.cell.sendValue(newValue);
    }
    ...
    set value(newValue) {
      ...
      if (newValue !== undefined) this.cell.value = newValue;
    }
    ...
    get readOnly() {
      return this.cell.readOnly;
    }
    get error() {
      return this.cell.error;
    }
    get pollMissError() {
      return this.cell.pollMissError;
    }
    get min() {
      return this.cell.min;
    }
    get max() {
      return this.cell.max;
    }
    get step() {
      return this.cell.step;
    }
    get valueType() {
      return this.cell.valueType;
    }
    get displayType() {
      return this.cell.displayType;
    }
    get order() {
  ...
  return {
    ...
    deleteDevice(deviceId) {
      ...
      const cellsTopics = allDeviceTopicBases.filter(topic => !deviceTopics.includes(topic));
    ...
    cell(id) {
      if (!cells.hasOwnProperty(id)) throw new Error('cell not found: ' + id);
      return cells[id];
    },
