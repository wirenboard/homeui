...
import { WarningTag, ErrorTag, BootstrapRow } from '../common';
...
function Port({ path, baud_rate, data_bits, parity, stop_bits }) {
  ...
  return (
    <Row title={t('device-manager.labels.port')}>
      {path} {baud_rate} {data_bits.toString()}
      {parity}
      {stop_bits.toString()}
    </Row>
  );
...
function DevicePanel(props) {
  var error;
  if (props.fw && props.fw.update) error = props.fw.update.error;
  ...
  return (
    <div key={props.uuid} className="panel panel-default">
      <div className="panel-body">
        <DeviceName
          title={props.title}
          bootloader_mode={props.bootloader_mode}
          online={props.online}
          poll={props.poll}
        />
