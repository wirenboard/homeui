...
function ScanningMessage({ports, count, ext}) {
  ...
  return (
    <>
      ...
      <InfoMessage msg={t(ext ? 'device-manager.labels.fast-scanning' : 'device-manager.labels.scanning', {ports})}/>
...
const DevicesPage = observer(({mqtt, scanning, devices, errors, onStartScanning, onStopScanning}) => {
  ...
  return (
    <>
      ...
      {scanInProgress && <ScanningMessage ports={scanning.scanningPorts.join(", ")} count={devices.devices.length} ext={scanning.isExtendedScanning}/>}
