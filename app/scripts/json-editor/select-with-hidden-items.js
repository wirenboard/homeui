...
function makeSelectWithHiddenItems() {
  return class extends JSONEditor.defaults.editors['select'] {
    enable() {
      ...
      // because enum_values can be populated later from enumSource
      if (!this.isEnabled() && this.enum_values.length && !this.enum_values.includes(this.value)) {
        // For editors with enumSource getDefault is not implemented
        this.setValue(this.enumSource ? this.enum_values[0] : this.getDefault());
      }
      super.enable();
    }
    ...
    hideFromChoices() {
      this.switcher_options = Array.from(this.theme.getSwitcherOptions(this.input));
