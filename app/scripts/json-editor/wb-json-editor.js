...
export function createJSONEditor(element, schema, value, locale, root) {
    ...
    element.addEventListener("input", (e) => {
        if (e.type == "textarea" || (e.type == "input" && (e.target.type=="text" || e.target.type=="ipv4"))) {
...
function overrideJSONEditor() {
    ...
    JSONEditor.defaults.resolvers.unshift(schema => schema.type === 'integer' && schema.format === 'siWb' && 'siWb');
    JSONEditor.defaults.resolvers.unshift(schema => schema.type === 'string' && schema.format === 'slWb' && 'slWb');
    JSONEditor.defaults.resolvers.unshift(schema => (schema.type === 'integer' || schema.type === 'string') && schema.format === 'edWb' && 'edWb');
    JSONEditor.defaults.resolvers.unshift(schema => schema.type === 'array' && schema.format === 'lazy-tabs' && 'lazy-tabs');
    JSONEditor.defaults.resolvers.unshift(schema => schema.type === 'array' && schema.format === 'collapsible-list' && 'collapsible-list');
    JSONEditor.defaults.resolvers.unshift(schema => schema.oneOf && schema.format === 'roMultiple' && 'roMultiple');
    JSONEditor.defaults.resolvers.unshift(schema => schema.oneOf && schema.format === 'wb-multiple' && 'wb-multiple');
    JSONEditor.defaults.resolvers.unshift(schema => schema.type === 'object' && schema.format === 'merge-default' && 'merge-default');
    JSONEditor.defaults.resolvers.unshift(schema => schema.type === 'object' && schema.format === 'wb-object' && 'wb-object');
    JSONEditor.defaults.resolvers.unshift(schema => schema.type === 'object' && schema.format === 'groups' && 'groups');
    JSONEditor.defaults.resolvers.unshift(schema => schema.format === 'unknown-device' && 'unknown-device');
    JSONEditor.defaults.resolvers.unshift(schema => schema.format === 'wb-optional' && 'wb-optional');
    JSONEditor.defaults.resolvers.unshift(schema => schema.type === 'array' && schema.format === 'wb-array' && 'wb-array');

    JSONEditor.defaults.editors["select"] = makeSelectWithHiddenItems();
    JSONEditor.defaults.editors["inWb"] = makeDisabledEditorWrapper(JSONEditor.defaults.editors["integer"]);
    JSONEditor.defaults.editors["nmWb"] = makeDisabledEditorWrapper(JSONEditor.defaults.editors["number"]);
    JSONEditor.defaults.editors["slWb"] = makeDisabledEditorWrapper(JSONEditor.defaults.editors["select"]);
