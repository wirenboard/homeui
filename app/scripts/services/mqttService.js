...
function whenMqttReady($q, $rootScope, mqttClient) {
  ...
  return () => {
    ...
    if (mqttClient.isConnected()) deferred.resolve();
    else {
      const unwatch = $rootScope.$watch(
        () => mqttClient.isConnected(),
        newValue => {
          if (!newValue) {
            // Resolve only when connection is established
            return;
          }
          deferred.resolve();
          unwatch();
...
function mqttClient(
  ...
  function clearReconnectTimeout() {
    if (reconnectTimeout !== null) $timeout.cancel(reconnectTimeout);
  }
  ...
  service.onConnectionLost = function (responseObject) {
    ...
    if (responseObject.errorCode !== 0) {
      // not intentionally disconnected
  ...
  service.onMessageDelivered = function (message) {
    if (message.qos > 0) {
      ...
      if (client.unsentMessagesQueue.length && client.availableSessionCapacity > 0) {
        client.unsentMessagesQueue.splice(0, client.availableSessionCapacity).forEach(message => {
