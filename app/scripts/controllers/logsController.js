...
class LogsCtrl {
      ...
      modalInstance.result.then(
        selectedDate => {
          if (!this.logDates.some(el => el.date === selectedDate.getTime())) {
            this.logDates.push({
              name: this.sinceMsg + ' ' + selectedDate.toLocaleString(),
  ...
  formatLogRow(l, service) {
    return (
      l.time.toISOString() + ' [' + this.removeServicePostfix(service || l.service) + '] ' + l.msg
    );
  }
  ...
  save() {
    ...
    const file = new Blob(
      [header, l.map(l => this.formatLogRow(l, this.selectedService.name)).join('\n')],
      { type: 'text/txt' }
    );
...
//-----------------------------------------------------------------------------
export default angular.module('homeuiApp.logs', []).controller('LogsCtrl', LogsCtrl);
